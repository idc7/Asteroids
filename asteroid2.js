...
function moveAll(objList, ctx){
   ...
   for(var i = 0; i < objList.length; i++){
      for(var j = i + 1; j < objList.length; j++){
         if(Math.sqrt((objList[i].locX - objList[j].locX ) * (objList[i].locX - objList[j].locX ) + (objList[i].locY - objList[j].locY) * (objList[i].locY - objList[j].locY)) < objList[i].radius + objList[j].radius){
...
function Asteroid(sLeft,sx,sy,lx,ly){ 
   ...
   this.locX = lx;
   this.locY = ly;
   this.direction = gRI(-5, 6);
   this.splitNum = 3;
   this.splitLeft = sLeft;
   this.radius = this.splitNum * this.splitNum + 3;
   this.isDead = false;
   //SpaceObj.call(this, sx, sy, lx, ly, this.splitNum * this.splitNum + 3);
   this.collide = function(objList, idx){
      if(objList[idx].getType === 'Ship' ){
         this.isDead = true;}
      if(objList[idx].getType === 'Pellet'){
         this.isDead = true;
         if(this.splitLeft > 1){
...
//   SpaceObj.call(this, 0, 0, 250, 250, 10);
   ...
   this.fire = function(spaceObjList){
      ...
      var psx = this.speedX + Math.sin(this.direction) * 10;
      var psy = this.speedY + Math.cos(this.direction) * 10; // 15 IS ARBETRARY FOR PELLET SPEED, TINKER WITH IT TO FINE TOON
...
//   SpaceObj.call(this, sx, sy, lx, ly, 2);
   ...
   this.draw = function(ctx){
      ...
      ctx.beginPath();
      ctx.arc(this.locX, this.locY, 2, 0, 2 * Math.PI);
      ctx.fill();
   };
   this.move = function(){
      this.moveLeft -= 1;
...
//   Pellet.inherits(SpaceObj);
   ...
   $(document).keydown(function(e) {
      switch(e.which) {
         ...
         case 38: // up
            moveList[0].changeSpeed(1);
         break;

         case 40: // down
            moveList[0].changeSpeed(-.2);
         break;
